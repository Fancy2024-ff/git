...
const nextActions = computed(() => {
  ...
  if (submissionReadiness.value?.warnings?.length) {
    for (const w of submissionReadiness.value.warnings) {
      actions.push({ type: 'agent', text: w })
    }
  }
  if (actions.length === 0 && submissionReadiness.value?.is_ready) {
    actions.push({ type: 'agent', text: '所有检查已通过，可以提交上架' })
  }
  return actions
})
</script>
    ...
    <div class="card">
      ...
      <div v-if="nextActions.length" class="card-body">
        <div v-for="(action, i) in nextActions" :key="i" class="action-item">
          <span class="action-type" :class="action.type === 'human' ? 'type--human' : 'type--agent'">
            {{ action.type === 'human' ? '人工' : 'Agent' }}
          </span>
...
.type--agent { background: var(--color-blue-subtle); color: var(--color-blue); }
