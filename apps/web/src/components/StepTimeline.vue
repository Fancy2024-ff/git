...
function getSubtitle(step: PipelineStep): string {
  return findStepDefinition(step.step || step.capability)?.nameEn || step.capability || ''
}
</script>
...
<template>
  <div class="timeline">
    <div
      ...
      :key="step.step || step.capability || idx"
      class="timeline-item"
      :class="{ 'timeline-item--active': selectedId === (step.step || step.capability) }"
      @click="emit('select', step.step || step.capability || '')"
    >
