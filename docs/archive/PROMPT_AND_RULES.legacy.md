# [历史文档] Prompt & Rules 说明（已归档）

> 🗄️ **历史文档，不代表当前架构和当前入口。**
> 本文的代码位置（`scripts/run_demo_pipeline.py:*`、`agents/*`）均已删除。
> 当前评分/分类规则在 `core/opportunity/`（scoring / viral_score / classifier），
> 增长策略在 `core/growth/`，QA 在 `core/qa/`，均为规则版 v1；
> 当前权威说明见各 `core/<域>/README.md`。
